...
## 2026-09-27 — 索引 4,501：near-me 主词进入稳定增长期

### 一、索引状态

| 指标             | 2026-09-19 | 2026-09-27 |                 变化 |
| ---------------- | ---------: | ---------: | -------------------: |
| GSC 已索引页面   |      2,662 |  **4,501** | **+1,839（+69.1%）** |
| Sitemap URL 总数 |      6,677 |      6,677 |                    — |
| 粗略索引率       |      39.9% |  **67.4%** |       +27.5 个百分点 |

> 索引增长继续强劲，已经覆盖约三分之二的 sitemap。当前不建议继续扩大 URL 数量，重点转向提升已索引页面的排名、CTR 和页面间主题关联。

### 二、GSC 过去 28 天表现

`Chart.csv` 当前最后日期为 2026-09-23，说明 9 月 26 日导出仍有约 3 天数据延迟。以下趋势使用完整日数据计算：

| 指标         | 09-10 至 09-16 | 09-17 至 09-23 |         变化 |
| ------------ | -------------: | -------------: | -----------: |
| Clicks       |             70 |        **124** |     **+77%** |
| Impressions  |         17,609 |     **28,909** |     **+64%** |
| CTR          |          0.40% |      **0.43%** |         略升 |
| 加权平均排名 |          12.46 |      **10.66** | 改善 1.80 位 |

累计可见趋势数据为 204 clicks、约 49,312 impressions、平均排名约 11.41；Queries/Pages 导出各自最多展示 1,000 行，因此不能将导出行汇总当作 GSC 总量。

### 三、关键词机会

| 查询                         |      展示 |   点击 | 平均排名 | 判断                            |
| ---------------------------- | --------: | -----: | -------: | ------------------------------- |
| `card shops near me`         | **4,104** | **12** | **7.59** | 核心词已稳定进入第一页          |
| `card shop near me`          |       768 |      4 |     8.54 | 继续争取前 5                    |
| `trading card shops near me` |       754 |      3 |     8.15 | 高价值主词                      |
| `card stores near me`        |       413 |      3 |     9.65 | 接近第一页底部                  |
| `sports card shops near me`  |       281 |      1 |     9.71 | 需要提升 CTR                    |
| `pokemon card shops near me` |       338 |      0 |     8.64 | 高展示零点击，优先优化标题/摘要 |

可见 `near me` 查询合计约 9,455 展示、32 clicks、加权平均排名约 9.22。相比 9 月 19 日的约 2,968 展示和 8 clicks，增长已经从测试信号变成稳定搜索需求。

### 四、页面表现与新机会

1. **游戏目录开始贡献真实流量**：`/directory/games/union-arena` 获得 89 展示、3 clicks、平均排名 8.38；`/directory/games/riftbound` 获得 152 展示、2 clicks、平均排名 11.08。Union Arena 是当前更明显的突破页面，应继续完善游戏页内容和州级交叉链接。
2. **目录页依然是高效率页面**：`/directory/ia` 获得 165 展示、3 clicks、平均排名 9.09；`/directory/ak/sitka` 21 展示、4 clicks、平均排名 6.43；`/directory/tn/johnson-city` 17 展示、3 clicks、平均排名 5.59。
3. **店铺页已经出现稳定的品牌/实体搜索**：`magic omens` 相关页面 223 展示、平均排名 7.05；`broken controller anderson` 页面 167 展示、平均排名 7.31。店铺页继续保持真实城市、游戏类别、营业时间和评分信息即可，不建议批量生成薄内容。
4. **`/near-me` 仍然没有成为主落地页**：页面本身约 31 展示、0 clicks、平均排名约 39。主词虽然增长，但 Google 仍主要把州页、城市页、游戏页和店铺页作为结果，需要继续增加上下文相关内链。
...
- Google：154 visitors，约 **90%**
- Bing：6 visitors，约 3%
- firsto.co：5 visitors，约 3%
- ChatGPT：2 visitors
- Brave、Yahoo、DuckDuckGo、GitHub：各少量流量

Google 已经成为绝对主流来源，SEO 流量已取代早期外链流量。页面访问方面，首页仍有 58 visitors，但 Ancient Tomb Cards、Riftbound、Union Arena、Orlando 等深层页面也持续获得访问，说明用户开始通过具体店铺、游戏和城市页面进入站点。
...
| 优先级 | 行动                                                                                                | 验收标准                                 |
| ------ | --------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| P0     | 部署本轮州页/游戏页到 `/near-me` 的内链修改，并请求 GSC 重新抓取 `/near-me`、Union Arena、Riftbound | `/near-me` 展示和排名在 2–4 周内明显改善 |
| P0     | 继续优化 `card shops near me` 和 `pokemon card shops near me` 的 title、description、首屏文案       | 主词 CTR 达到 1% 以上，排名向前 5 移动   |
| P1     | 为 Union Arena、Riftbound 游戏页增加更强的游戏介绍、热门州/城市链接                                 | 游戏页保持前 10–15，并增加点击           |
| P1     | 对 GSC 高展示零点击页面进行页面级优化，而不是批量改所有店铺页                                       | 每周优先处理展示量最高的 10–20 个页面    |
| P2     | 继续观察 4,501 已索引页对应的 Crawled - currently not indexed 和重复页面                            | 索引率继续上升且不引入低质量 URL         |
| P2     | 继续使用 IndexNow 提交本轮更新页面                                                                  | Bing near-me 关键词展示继续增长          |
